...
def write_FEFFinp(template_dir,pos_filename,CA,site,ipot_dist,radius,numbers=0):
    ...
    with open(f"FEFF_inp//{title}.inp",'w') as f:
        ...
        f.write(calc_pot_atoms_list(pos_filename, CA,ipot_dist=ipot_dist,cluster_size=radius)[site]["potential"])
        f.write('\n\n')
        f.write("ATOMS\n")
        f.write(calc_pot_atoms_list(pos_filename, CA,ipot_dist=ipot_dist,cluster_size=radius)[site]["atoms"])
        f.write('\n')
        f.write("END\n")
...
class FEFF_cal:
    ...
    def FEFFinp_gen(self):
        self.inp_file,self.title=write_FEFFinp(self.template_dir,self.pos_filename,self.CA,self.site,ipot_dist=self.ipot_dist,radius=self.radius,number=self.numbers)
...
def main():
    ...
    if args.run_file is True and restart is False:
        readfiles=glob.glob("FEFF_inp/*.inp")
        if isinstance(readfiles, str):
            readfiles=[readfiles]
        FEFF_obj=[]
    ...
    if args.run_file is True and restart is True:
        readfiles=glob.glob("FEFF_inp/*.inp")
        readout=glob.glob("output/*.json")
        out = []
        input = []
        for str1 in readout:
